...
class _AudioPageState extends State<AudioPage> {
  ...
  Future showAlertDialog() async {
    SharedPreferences checkDialog = await SharedPreferences.getInstance();
    String? firstTime = checkDialog.getString('firstTime');
    if (firstTime == "no") {
    } else {
      checkDialog.setString('firstTime', 'no');
      return showDialog(
          context: context,
          builder: (BuildContext context) {
            return AlertDialog(
              ...
              content: Text(
                'balayez vers la gauche ou la droite pour vous déplacer entre les mystères',
  ...
  moveToNextAudio(int number) {
    switch (number) {
      case 1:
        {
          return Navigator.pushReplacementNamed(context, '/audiopage',
              arguments: MysteryFields(
                audioFile: '${mydir?.path}/annonciation.mp3',
                videoFile: '${mydir?.path}/1-annonciation.mp4',
                textNo: 1,
                minAudio: widget.minAudio,
                maxAudio: widget.maxAudio,
      ...
      case 5:
        return Navigator.pushReplacementNamed(context, '/audiopage',
            arguments: MysteryFields(
              ...
              audioText: Text('There was a problem opening this document',
                  style: TextStyle(color: black)),
            ));
      case 6:
        return Navigator.pushReplacementNamed(context, '/audiopage',
            arguments: MysteryFields(
              ...
              videoFile: '${mydir?.path}/6-agonie_not-stretched.mp4',
              textNo: 6,
              minAudio: widget.minAudio,
              maxAudio: widget.maxAudio,
              title: "L'Agonie",
  ...
  void initState() {
    ...
    getDir().then((value) {
      assetsAudioPlayer.open(
        Audio.file((widget.audioFile) as String),
        autoStart: false,
        audioFocusStrategy:
      ...
      _controller = VideoPlayerController.file(
          File((widget.videoFile) as String),
        ...
        : GestureDetector(
            onPanUpdate: (details) {
              ...
              if (details.delta.dx > 0) {
                int number = (widget.textNo!) - 1;
                if ((number) < widget.minAudio!) {
                } else {
                  moveToNextAudio(number);
                }
              }
